...
bool COverlappedWindow::RegisterClass()
{
	...
	wind.hIcon = LoadIcon( hInstance, MAKEINTRESOURCE( IDI_APPLICATION ) );
	...
	wind.hIconSm = LoadIcon( hInstance, MAKEINTRESOURCE( IDI_APPLICATION ) );  
...
bool COverlappedWindow::Create()
{
	...
	if (!handle) { 
		MessageBox( NULL, _T("Call to CreateWindow failed!"), _T("Ellipse"), NULL ); 
		return false;
	} 
...
LRESULT CALLBACK COverlappedWindow::windowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_NCCREATE) {
		...
		COverlappedWindow* thisWnd = (COverlappedWindow*)tmp->lpCreateParams;
		::SetWindowLongPtr(hwnd, GWL_USERDATA, (LONG_PTR)thisWnd);
		thisWnd->handle = hwnd;
		return 1;
	}
